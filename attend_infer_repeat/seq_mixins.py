...
from ops import select_present_list, extract_state
...
#         make_ta('prop_prob', [self.max_steps + 1])
...
#         highest_used_id = -tf.ones((self.effective_batch_size, 1))
...
#                      highest_used_id, prior_hidden_state, obj_num_counts] + tas
#
#         return loop_vars
#
...
#
#     def _loop_body(self, t, img_seq, state, time_state, cumulative_imp_weights, prev_ids, highest_used_id,
...
#         #
#         hidden_outputs, state, time_state, current_ids, highest_used_id, prior_hidden_state, z_t,\
#         disc_num_steps_per_sample, prop_num_steps_per_sample, disc_presence_prob, prop_presence_prob \
#             = self._propagate(z_tm1, old_hidden_state, time_state, prev_ids, highest_used_id, prior_hidden_state)
#
#         # extract latents
...
#         prop_num_step_prior_probs, prop_posterior_prob, disc_posterior_prob \
#             = self._compute_elbo(t, img, canvas, hidden_outputs, z_tm1, obj_num_counts, disc_num_steps_per_sample,
...
#         # write outputs
#         tas = list(tas)
#         outputs = hidden_outputs + [canvas, glimpse, likelihood, kl_what, kl_where, kl_prop_steps,
#                                     kl_disc_steps, kl_steps, kl, elbo, prop_num_steps_per_sample,
#                                     disc_num_steps_per_sample, num_step_per_sample, flat_iw, iw_elbo] + prior_stats +\
#                                    [prop_num_step_prior_probs, prop_posterior_prob, disc_posterior_prob]
#
...
#         t += 1
#         return [t, img_seq, state, time_state, cumulative_imp_weights, current_ids, highest_used_id,
#                 prior_hidden_state, obj_num_counts] + tas
#
...
#         return time_state
#
#     def _propagate(self, z_tm1, prev_hidden_state, time_state, prev_ids, highest_used_id, prior_hidden_state):
...
#         return hidden_outputs, state, time_state, prev_ids, highest_used_id, prior_hidden_state, z_tm1,\
...
#                 return x * p + (1. - p) * eps
#
#             prior_what_loc, prior_where_loc = (mask(i, presence_tm1) for i in (prior_what_loc, prior_where_loc))
#             prior_what_scale, prior_where_scale = (mask(tf.nn.softplus(i), presence_tm1, 1e-8)
#                                                    for i in (prior_what_scale, prior_where_scale))
#         else:
...
#
#     def _compute_elbo(self, t, img, canvas, hidden_outputs, z_tm1, obj_num_counts, disc_num_steps_per_sample,
...
#
#         prop_posterior_prob = prop_num_steps_posterior.probs
#         disc_posterior_prob = disc_num_steps_posterior.probs
#
...
#         kl_prop_steps *= tf.to_float(tf.greater(t, 0))
#
#         kl_steps = kl_disc_steps + kl_prop_steps
#         # TODO: kl steps is now optimsed by two paths: REINFORCE and analytically w/o sampling. Verify!
#         kl_steps = tf.stop_gradient(kl_steps)
#
#         ### KL
...
#                num_step_per_sample, prop_num_step_prior_probs, prop_posterior_prob, disc_posterior_prob
#
#     def _time_loop(self):
...
#
#         # # TODO: verify!
#         # prop_pres_logit = extract_state(propagate_outputs, 8)
#         # prop_log_prob = tf.nn.sigmoid_cross_entropy_with_logits(labels=prop_pres, logits=prop_pres_logit)
#
...
#         discovery_log_prob = NumStepsDistribution(disc_presence_prob[..., 0]).log_prob(disc_num_steps_per_sample[..., tf.newaxis])
#         step_log_prob = prop_log_prob + discovery_log_prob[..., tf.newaxis]
#
#         highest_used_id, new_obj_id = compute_object_ids(highest_used_id, prev_ids, prop_pres, disc_pres)
#
...
#         return hidden_outputs, state, time_state, obj_ids, highest_used_id, prior_hidden_state, z_tm1,\
#                disc_num_steps_per_sample, prop_num_steps_per_sample, disc_presence_prob, prop_presence_prob


from refactor import AttendPropagateRepeat, AttendDiscoverRepeat, APDR, _extract_latents


class SeparateSeqAIRMixin(object):
    ...
    def _make_cells(self, transition, input_encoder, glimpse_encoder, transform_estimator, steps_predictor,
        ...
        self.prior_rnn = self.prior_rnn_class(self._n_hidden)
        self.propagate = AttendPropagateRepeat(self.max_steps, self.effective_batch_size, propagation_cell,
                                               self.prior_rnn)

        self.time_transition = self.time_transition_class(self._n_hidden)
    ...
    def _initial_state(self):
        ...
        loop_vars = [t, self.used_obs, self.z0, time_state, cumulative_imp_weights, prev_ids,
                     last_used_id, self.init_prior_prop_state, obj_num_counts]
    ...
    def _loop_body(self, t, img_seq, z_tm1, time_state, cumulative_imp_weights, prev_ids, last_used_id,
        ...
        prop_prior_state = [prior_hidden_state, obj_num_counts]
        apdr_outputs = self.apdr(img, z_tm1, time_state, prop_prior_state, last_used_id, prev_ids)
        hidden_outputs = apdr_outputs.hidden_outputs
        z_t = [what, where, presence, pres_logit] = _extract_latents(hidden_outputs, skip='pres_prob'.split())
        ...
        # Increment time index
        t += 1
        prior_hidden_state, obj_num_counts = apdr_outputs.prop_prior_state
